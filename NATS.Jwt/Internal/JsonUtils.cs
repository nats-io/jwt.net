...
namespace NATS.Jwt.Internal
{
    internal static class JsonUtils
    {
        ...
        public static DurationJson AsDuration(JSONNode node, string field, DurationJson dflt)
        {
            if (dflt == null)
            {
                long l = node.GetValueOrDefault(field, long.MinValue).AsLong;
                return l == long.MinValue ? null : DurationJson.OfNanos(l);
            }
            return DurationJson.OfNanos(node.GetValueOrDefault(field, dflt.Nanos).AsLong);
        }
        ...
        public static List<DurationJson> DurationList(JSONNode node, string field, bool nullIfEmpty = false)
        {
            List<DurationJson> list = new List<DurationJson>();
            foreach (var child in node[field].Children)
            {
                list.Add(DurationJson.OfNanos(child.AsLong));
            }
        ...
        public static void AddField(JSONObject o, string field, DurationJson value)
        {
        ...
        public static void AddField(JSONObject o, string field, IList<DurationJson> values)
        {
            if (values != null && values.Count > 0)
            {
                JSONArray ja = new JSONArray();
                foreach (DurationJson d in values)
                {
                    ja.Add(d.Nanos);
                }
                o[field] = ja;
