...
public static class NatsJwt
{
    ...
    /// <summary>
    /// Maximum accepted size of an encoded JWT string, in characters.
    /// </summary>
    public const int MaxTokenSize = 1024 * 1024;
    ...
    public static string FormatUserConfig(string jwt, string seed)
    {
        ...
        {
            throw new NatsJwtException($"JWT exceeds maximum allowed size of {MaxTokenSize} bytes");
        }
        ...
        try
        {
            kp = KeyPair.FromSeed(seed.AsSpan());
        }
        catch (Exception ex)
        {
            throw new NatsJwtException("Invalid seed: " + ex.Message, ex);
        }

        var seedKind = kp.Prefix switch
        {
        ...
        where T : JwtClaimsData
    {
        ...
        {
            throw new NatsJwtException("Invalid JWT: " + ex.Message, ex);
        }
        ...
        static void VerifyClaims(JwtClaimsData claimsData, string payload, byte[] signature)
        {
            ...
            PrefixByte[] expectedPrefixes = claimsData.ExpectedPrefixes();
            if (expectedPrefixes.Length > 0 && !expectedPrefixes.Any(p => KeyPair.IsValidPublicKey(p, issuer.AsSpan())))
            {
                throw new NatsJwtException($"Invalid issuer key for {claimsData.GetType().Name}: expected one of '{string.Join(",", expectedPrefixes)}'");
            }
